...
run_pcair <- function(analysis_name){
  ...
  spinny$spin()
  tryCatch({
    snpgdsBED2GDS(bed.fn = bed_file, bim.fn = bim_file, fam.fn = fam_file, 
                  out.gdsfn = gds_file)
    spinny$finish()
    cli::cli_alert_success("Conversion complete.")
  }, error = function(e) {
    spinny$finish()
    cli::cli_alert_danger("Error during BED to GDS conversion: " %&% e$message)
  ...
  tryCatch({
    ...
    cli::cli_alert_danger("Error opening GDS file: " %&% e$message)
    stop("Failed to open GDS file.")
  })

  # If you have already pruned variants just run the function as it is otherwise remove the hashtag
  load(out_dir %&% analysis_name %&%  "_snpset.RData")
  ...
  diag(KINGmat2) <- 0.5
  KINGmat_dense <- as.matrix(KINGmat2)
  save(KINGmat_dense, file = out_dir %&% analysis_name %&% "_kingmat_results.RData")
  ...
  spinny$spin()
  tryCatch({
    pcair_res <- pcair(gds, kinobj = KINGmat_dense, divobj = KINGmat_dense, snp.include = pruned, num.cores = 16)
  ...
  snpgdsClose(gds)
  rm(gds, KINGmat_dense, pcair_res, pruned)
  gc()
}
