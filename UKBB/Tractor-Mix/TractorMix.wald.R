...
TractorMix.wald <- function(fixed, data = parent.frame(), kin = NULL, id, family = binomial(link = "logit"), infiles, method = "REML", method.optim = "AI", maxiter = 500, tol = 1e-5, taumin = 1e-5, taumax = 1e5, tauregion = 10, missing.method = "impute2mean", verbose = FALSE, ...) {
  ...
  dosage_id_lst = lapply(files, function(file){
    fileSplit = unlist(strsplit(file[1], "\t"))
    as.character(fileSplit[6:length(fileSplit)])
  })
  phe_GRM_id_lst = list(data[[id]], row.names(kin), colnames(kin))
  all_id_lst = append(phe_GRM_id_lst, dosage_id_lst)
  if(!all(unlist(lapply(all_id_lst[-1], identical, all_id_lst[[1]])))) stop("Error: sample IDs should match in \"data\", \"kin\", and \"infiles\".")
  ...
  idx <- match(rownames(model.frame(formula = as.formula(deparse(fixed)), data = data, na.action = na.omit)),
               rownames(model.frame(formula = as.formula(deparse(fixed)), data = data, na.action = na.pass)))
  ...
  for (i in 1:nSNP){
    ...
    tmpkins = list(kins1 = kin[idx, idx])
